    ...
    public class Main : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                XYZ offset1 = roomCenter1 + offset;

                Transaction transaction = new Transaction(doc, "Копирование группы");
                transaction.Start();
                doc.Create.PlaceGroup(offset1, group.GroupType);
                transaction.Commit();
            }
